...
while [[ $i -le $num ]]; do
   ...
   #valtime=`${ndate} +${hh} ${PDY_dir}${strthr}`
...
#        $hh -eq 15 && $cyc -eq 06 ]; then
   ...
   pyplotdir=${pyplot_dir}/pyplot_work_${obstype}.${valpdy}_${hr}
   mkdir -p $pyplotdir

   figout=${figout_dir}/pyplot_nest_${obstype}.${valpdy}
   mkdir -p $figout
   cd ${pyplotdir}
